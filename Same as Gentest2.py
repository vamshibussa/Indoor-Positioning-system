...
#train_val_split2 = np.random.rand(len(finalfeatures)) < 0.70
test_dataset2 = pd.read_csv("test3.csv",header = None)
...
regularizer = tf.nn.l2_loss(dnn2_weights_h1)+tf.nn.l2_loss(dnn2_weights_h2)+tf.nn.l2_loss(dnn2_weights_out)
cost = tf.reduce_mean(s_cost_function2 + lamda * regularizer)
train = tf.train.MomentumOptimizer(learning_rate2,0.9).minimize(cost)
...
with tf.Session() as session:
    ...
    for epoch in range(training_epochs2):
        ...
        for b in range(total_batches2):
            offset = (b * batch_size2) % (train_x2.shape[0] - batch_size2)
            batch_x = train_x2[offset:(offset + batch_size2), :]
            batch_y = train_y2[offset:(offset + batch_size2), :]
            _, c = session.run([train, cost],feed_dict={X2: batch_x, Y2 : batch_y})
            epoch_costs = np.append(epoch_costs,c)
    ...
    # Instead of setting number of iterations we can also find where validation error starts increasing for generalization
    print("\nTesting Cost Function:", session.run(cost, feed_dict={X2: testfinalfeatures , Y2: testlabels2}))
...
'''
regularizer = tf.nn.l2_loss(weights['h1'])+tf.nn.l2_loss(weights['h2'])+tf.nn.l2_loss(weights['h3'])+tf.nn.l2_loss(weights['h4'])+tf.nn.l2_loss(weights['h5'])+tf.nn.l2_loss(weights['h6'])
cost = tf.reduce_mean(cost + lamda * regularizer)
train = tf.train.MomentumOptimizer(lr,0.9).minimize(cost)
'''
